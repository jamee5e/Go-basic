...
type usersUsecase struct {
	cfg             config.IConfig
	usersRepository usersRepositories.IUsersReposity
}

func UsersUsecase(cfg config.IConfig, usersRepository usersRepositories.IUsersReposity) IUsersUsecase {
	return &usersUsecase{
		cfg:             cfg,
...
func (u *usersUsecase) GetPassport(req *users.UserCredential) (*users.UserPassport, error) {
	...
	// Sign token
	accessToken, err := jameauth.NewJameAuth(jameauth.Access, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
	refreshToken, err := jameauth.NewJameAuth(jameauth.Refresh, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
...
func (u *usersUsecase) RefreshPassport(req *users.UserRefreshCredential) (*users.UserPassport, error) {
	// Parse token
	claims, err := jameauth.ParseToken(u.cfg.Jwt(), req.RefreshToken)
	if err != nil {
		return nil, err
	}
	...
	accessToken, err := jameauth.NewJameAuth(
		jameauth.Access,
		u.cfg.Jwt(),
		newClaims,
	)
	...
	refreshToken := jameauth.RepeatToken(
		u.cfg.Jwt(),
		newClaims,
